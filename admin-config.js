...
// Helper: panel de moderación manual de fotos de perfil
function renderAvatarsConfig(){
  const usersWithPhoto = (D.users||[]).filter(u=>u.avatar_url);

  if(!usersWithPhoto.length){
    return `<div style="text-align:center;padding:40px;color:var(--textm)">
      <i data-lucide="image-off" style="width:40px;height:40px;margin:0 auto 10px;display:block;opacity:.5"></i>
      Nadie tiene foto de perfil subida todavía.
    </div>`;
  }

  return `
  <div style="margin-bottom:14px">
    <p style="font-size:13px;color:var(--textm)">Revisa las fotos de perfil subidas por los usuarios. Si alguna es inapropiada, elimínala — el usuario quedará con el avatar por defecto y podrá subir una nueva.</p>
  </div>
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:14px">
    ${usersWithPhoto.map(u=>`
      <div style="background:var(--surface);border-radius:12px;padding:10px;text-align:center">
        ...
        <p style="font-size:10px;color:var(--textm);margin-bottom:8px">${u.email||''}</p>
        <button onclick="deleteUserAvatar('${u.id}')" style="width:100%;background:#7f1d1d;color:#fff;border:none;border-radius:8px;padding:7px;font-size:12px;font-weight:600;cursor:pointer">
          🗑️ Eliminar foto
        </button>
      </div>
    `).join('')}
...
// Elimina la foto de perfil de un usuario (moderación manual por admin)
