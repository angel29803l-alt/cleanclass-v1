...
// 🔧 Moderación de imágenes: carga el modelo nsfwjs una sola vez (se reutiliza)
let _nsfwModel = null;
async function _getNsfwModel(){
  if(_nsfwModel) return _nsfwModel;
  _nsfwModel = await nsfwjs.load(); // usa el modelo por defecto (liviano, corre en el navegador)
  return _nsfwModel;
}

// Analiza un archivo de imagen y devuelve {safe, reason, predictions}
async function _checkImageIsSafe(file){
  try{
    ...
    const getP = (name)=> predictions.find(p=>p.className===name)?.probability || 0;
    const pornP = getP('Porn');
    const hentaiP = getP('Hentai');
    const sexyP = getP('Sexy');

    // Umbral: bloquea si detecta pornografía/hentai con confianza media-alta,
    // o si "sexy" + pornografía combinadas son altas.
    const safe = pornP < 0.5 && hentaiP < 0.5 && (pornP + hentaiP + sexyP) < 0.75;
    return { safe, reason: safe?null:'Se detectó contenido inapropiado en la imagen.', predictions };
  }catch(err){
    console.error('No se pudo analizar la imagen con el moderador:', err);
    // Si el modelo falla en cargar (ej. sin internet), dejamos pasar la imagen
    // para no bloquear la app por completo, pero queda el panel manual como respaldo.
    return { safe: true, reason: null, predictions: [] };
  }
}
...
function updateProfileImage(input){
  ...
  n.textContent='🔎 Revisando la imagen...';
  document.body.appendChild(n);

  _checkImageIsSafe(file).then(async(result)=>{
    if(!result.safe){
      n.style.background='#7f1d1d';
      n.textContent='⛔ Esta foto no se puede usar como foto de perfil';
      setTimeout(()=>n.remove(),3500);
      input.value=''; // limpia el input para que no quede el archivo rechazado seleccionado
      return;
    }
